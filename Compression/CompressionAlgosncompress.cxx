...
//Y plane
  ...
  void CompressionAlgosncompress::ApplyCompression(const std::vector<short> &waveform, const int mode, const UInt_t ch){
    ...
    thr = _thresh[ch];
    //std::cout << "channel " << ch << "threshold is " << thr << std::endl;
...
while(thresh_file.good()){
     ...
     _thresh[a-1] = b;
   //    std::cout << "channel " << a << " threshold= " << b << " c " << c << " d " << d << " e " << e << " f " << f << std::endl;
}

for(int hhg=0; hhg < 5; hhg++){
//std::cout << "first vector " << _thresh[hhg] << std::endl;
}

for(int jjj=2500; jjj<2505; jjj++){
//std::cout<< "bob " << _thresh[jjj] << std::endl;
}

for( int kk=4800; kk<4805 ; kk++){
//std::cout << "steve " << _thresh[kk] << std::endl;
}
